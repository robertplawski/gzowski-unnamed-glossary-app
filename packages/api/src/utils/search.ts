 ...
 */
function calculateSemanticScore(entry: Record<string, any>, query: string): number {
	const semanticTerms = extractSemanticTerms(entry);
	const lowerQuery = query.toLowerCase();
	
	// Check if query matches any semantic terms
	const hasSemanticMatch = semanticTerms.some(term => 
		term.includes(lowerQuery) || lowerQuery.includes(term)
	);
...
export function analyzeEntryMatch(
	...
	for (const { field, weight } of searchableFields) {
		...
		// 4. Substring match with restrictions for short queries (40 points)
			...
			if (query.length < SEARCH_CONFIG.SHORT_WORD_THRESHOLD) {
				...
				const hasMeaningfulSubstring = words.some(word => {
					return word.startsWith(lowerQuery) || 
						   word.endsWith(lowerQuery) || 
						   (word.includes(lowerQuery) && Math.abs(word.length - query.length) <= 3);
				});
		...
		if (fieldMatchType) {
			matchTypeScores[fieldMatchType] = (matchTypeScores[fieldMatchType] || 0) + fieldScore;
		}
	...
	if (totalScore > 0) {
		const sortedMatchTypes = Object.entries(matchTypeScores)
			.sort(([,a], [,b]) => b - a);
 ...
 */
export function getSearchStats<T>(searchResults: SearchResult<T>[]): SearchStats {
